...
then
    ffmpeg -i "$inputfile" -c:v libx265 -preset:v $preset -crf $crf $audioencoding $scaleresult -pix_fmt yuv420p delivery_x265.mov
else
    ffmpeg -i "$inputfile" -c:v libx265 -preset:v $preset -crf $crf $audioencoding $scaleresult delivery_x265.mov
fi
